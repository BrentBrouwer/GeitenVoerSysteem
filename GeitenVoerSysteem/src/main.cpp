...
/*#motorButton{background-color: #28a745;} // Default for initial page load */
...
// The footer now includes the JavaScript polling logic
...
// --- Handle Alive LED (No change needed) ---
...
{
    // Pre-allocate buffer for dynamic parts (increased safety)
    char buffer[2048];
    ...
    // Note: Initial status is based on current ESP32 state, JS takes over quickly
...
void handleStatus()
{
    ...
    // response += ",\"duration\":";
    // response += motorDurationMs;
    // response += "}";
    server.send(200, "application/json", response);
}

// --- Handler Functions (No changes needed here for functionality) ---
...
void setup()
{
    ...
    server.on("/status", handleStatus); // <-- NEW API ENDPOINT
...
void loop()
{
    ...
    if (isMotorRunning && motorStartTime > 0)
    {
        ...
        {
            // Motor stops here, isMotorRunning is set to false
            digitalWrite(MOTOR_PIN, LOW);
            isMotorRunning = false;
            motorStartTime = 0;
            char msg[100];
            ...
            // Note: The JavaScript polling handles the visual update from now on.
        }
    }
}